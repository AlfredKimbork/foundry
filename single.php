    ...
    <article class="mx-auto max-w-7xl">
      <?php while (have_posts()) : the_post(); ?>
        ...
        <header class="mb-8">
          ...
          <!-- Title -->
          <h2 class="mt-3 text-4xl font-bold tracking-tight text-text sm:text-5xl lg:text-6xl">
            <?php the_title(); ?>
          </h2>
        ...
        <nav class="mt-12 grid gap-4 border-t border-border pt-8 sm:grid-cols-2" aria-label="Post navigation">
          ...
          <div>
            ...
            <?php if ($previous): ?>
              <a href="<?php echo esc_url(get_permalink($previous)); ?>" class="group block rounded-xl border border-border bg-card p-5 transition duration-200 hover:-translate-y-0.5 hover:border-accent hover:shadow-md">
                <span class="text-sm font-medium text-text-secondary">← Previous post</span>
          ...
          <div>
            ...
            <?php if ($next): ?>
              <a href="<?php echo esc_url(get_permalink($next)); ?>" class="block rounded-xl border border-border bg-card p-5 text-left transition duration-200 hover:-translate-y-0.5 hover:border-accent hover:shadow-md sm:text-right">
                <span class="text-sm font-medium text-text-secondary">Next post →</span>
                <span class="mt-2 block text-lg font-semibold tracking-tight text-text transition-colors group-hover:text-accent">
